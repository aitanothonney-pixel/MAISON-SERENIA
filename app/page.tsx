        ...
        <div className={`relative overflow-hidden bg-white aspect-[4/3] ${product.name.includes('Bubble') || product.category === 'Figurines' ? 'p-4' : ''}`}>
          ...
          {isBubble && product.images[1] && (
            <Image
              ...
              width={600}
              height={450}
              className={`absolute inset-0 w-full h-full opacity-0 group-hover:opacity-100 transition-all duration-700 group-hover:scale-105 object-contain ${[12, 13, 10].includes(product.id) ? 'scale-[1.4]' : [2, 7, 9, 22].includes(product.id) ? 'scale-125' : ''}`}
            />
          )}
